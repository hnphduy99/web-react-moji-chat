...
const GroupChatAvatar = ({ participants, type }: { participants: Participant[]; type: 'chat' | 'sidebar' }) => {
  const avatars = [];
  ...
  for (let i = 0; i < limit; i++) {
    ...
    avatars.push(
      <UserAvatar key={i} type={type} name={member.displayName} avatarUrl={member.avatarUrl ?? undefined}></UserAvatar>
    );
  }
